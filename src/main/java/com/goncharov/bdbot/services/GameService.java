...
public class GameService {
    private static String forMafia = "Твоя роль: Мафия\nТвоя задача убить жителей и забрать их карточку, только делай это скрытно иначе тебя может посадить комиссар." +
            "Специально для тебя: можешь узнать три твоих цели по команде /victims";

    private static String forCitizen = "Твоя роль: Мирный житель\nТы чисто бегунок и попуск. Делай задания и отмечай в таблице суетись и тд и тп, я проверю потом." +
            "Кстати, не попадись мафии и комиссару. Первый убьет тебя лоха, а второй откатит задание, так что все нужно делать скрытно." +
            "Кстати, список заданий мирных жителей можешь получить по команде /tasks";
    ...
    public String addUsername(int id, String username) {
